...
% 
%     for i1 = 1:len
%         observer_errors(i1, :) = pdist2(broadcast_history{i0}(i1, 1:2), store_states{i0}(i1, 1:2),'euclidean');
%     end
% 
%     % 绘制每个无人艇的 'u', 'v', 'phi' 轨迹
%     plot(Time(1:len), observer_errors(1:len), 'Color', colors(i0, :)); % 'u' 轨迹
%     
%     h =  plot(NaN, NaN, '-', 'Color', colors(i0, :), 'MarkerSize', 10, 'LineWidth', 1.5, 'DisplayName', 'Observer Error');
%     % 设置图例，每个图窗的图例只显示一次
%     legend(h, 'NumColumns', 1); % 设置图例，并将 NumColumns 改为 1
% 
%     % 设置图形属性
%     set(gca,'FontSize',14);
%     xlabel('${t (s)}$','Interpreter','latex');
%     ylabel(sprintf('$$\\left\\|p_{%d} - p_{%d}^{o}\\right\\|| \\ (m)$$', i0, i0), 'Interpreter', 'latex');
% end
